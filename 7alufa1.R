...
election <- simmer("election")%>%
  ...
  add_resource("center_stand_left",capacity=13, queue_size=0)
...
mm1envs <- mclapply(1:25, function(i) {
  set.seed(9*i+100)
...
el3_1 <- get_mon_attributes(mm1envs)

vote_to_left_prob1 <-sqldf("select value as left_voters_prob_global
                    from el3_1
                   where key=='KPI1'
                    group by replication")[,1]

reneged_from_left_prob1 <-sqldf("select value as reneged_from_left_do5n
                    from el3_1
                   where key=='KPI2'
                    group by replication")[,1]
...
data1 <- data.frame(vote_to_left_prob1,reneged_from_left_prob1)

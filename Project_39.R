...
#this func to chick if the voter renged from a left do5n or not
renege_left <- function(resource){
...
election <- simmer("election")%>%
  ...
  add_resource("center_stand_left",capacity=10, queue_size=0)
...
mm0envs <- mclapply(1:25, function(i) {
  set.seed(9*i+100)
...
el3_0 <- get_mon_attributes(mm0envs)

vote_to_left_prob0 <-sqldf("select value as left_voters_prob_global
                    from el3_0
                   where key=='KPI1'
                    group by replication")[,1]

reneged_from_left_prob0 <-sqldf("select value as reneged_from_left_do5n
                    from el3_0
                   where key=='KPI2'
                    group by replication")[,1]
...
data0 <- data.frame(vote_to_left_prob0,reneged_from_left_prob0)

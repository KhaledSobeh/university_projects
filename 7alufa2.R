...
initialOpinion_road_probFunc_7alufa2 <- function(){#Step 3
  ########################################## get initial option
  initial_opinion <- 0
  x <- runif(1,0,1)
  if(x<0.5){#right
  ...
  if(initial_opinion==1){
    return(c(as.integer(initial_opinion),0.14-z/2,0.39-z/2,0.47+z))
  }
  else if(initial_opinion==2){
    return(c(as.integer(initial_opinion),0.14+z,0.39-z/2,0.47-z/2))
  }
  else{
    return(c(as.integer(initial_opinion),0.14-z/2,0.39+z,0.47-z/2))
  }
}
...
election <- simmer("election")%>%
  ...
  add_resource("south_stand_left",capacity=3, queue_size=0) %>%
  ...
  add_resource("north_stand_left",capacity=5, queue_size=0) %>%
  ...
  add_resource("center_stand_left",capacity=11, queue_size=0)
...
center_compound_do5n3 <- trajectory("center_compound_do5n3") %>%
  ...
  renege_in(
    t = function()isRenege(get_attribute(election,"right_prob_origin"),get_attribute(election,"center_prob_origin"),
                           get_attribute(election,"left_prob_origin"), get_attribute(election,"place2")), out = center_compound_do5n3_renege) %>%
  timeout(function () runif(1, 2, 6)) %>%
  renege_abort() %>%
...
center_compound_do5n2 <- trajectory("center_compound_do5n2") %>%
  ...
  renege_in(
    t = function()isRenege(get_attribute(election,"right_prob_origin"),get_attribute(election,"center_prob_origin"),
                           get_attribute(election,"left_prob_origin"), get_attribute(election,"place2")), out = center_compound_do5n2_renege) %>%
  timeout(function () runif(1, 2, 6)) %>%
  renege_abort() %>%
...
center_compound_do5n1 <- trajectory("center_compound_do5n1") %>%
  set_attribute(key=c("initial_opinion","right_prob","center_prob","left_prob"),
                value=function() initialOpinion_road_probFunc_7alufa2())%>%
  set_attribute(key=c("right_prob_origin","center_prob_origin","left_prob_origin"),
                value=function() c(get_attribute(election,"right_prob"),get_attribute(election,"center_prob"),get_attribute(election,"left_prob")))%>%
  set_attribute(key = c("place1","place2","place3"), 
                value = function() get_places(get_attribute(election,"initial_opinion"))) %>%
  ...
  renege_in(
    t = function()isRenege(get_attribute(election,"right_prob_origin"),get_attribute(election,"center_prob_origin"),
                           get_attribute(election,"left_prob_origin"), get_attribute(election,"place1")), out = center_compound_do5n1_renege) %>%
  timeout(function () runif(1, 2, 6)) %>%
  renege_abort() %>%
...
mm2envs <- mclapply(1:25, function(i) {
  set.seed(9*i+100)
...
el3_2 <- get_mon_attributes(mm2envs)

vote_to_left_prob2 <-sqldf("select value as left_voters_prob_global
                    from el3_2
                   where key=='KPI1'
                    group by replication")[,1]

reneged_from_left_prob2 <-sqldf("select value as reneged_from_left_do5n
                    from el3_2
                   where key=='KPI2'
                    group by replication")[,1]
...
data2 <- data.frame(vote_to_left_prob2,reneged_from_left_prob2)
